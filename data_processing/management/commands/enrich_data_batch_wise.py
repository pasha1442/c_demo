...
from data_processing.models import DataEnrichment
from data_processing.services.data_enricher_service import DataEnricherService
...
class Command(BaseCommand):
    help = 'Process data enrichment requests in batches'


    def handle(self, *args, **options):

        with start_transaction(op="DjangoCommand", name="DataEnricher-Dev02"): 
            self.runhandle()
    ...
    def runhandle(self, *args, **options):
        ...
        pending_requests = DataEnrichment.without_company_objects.filter(
            status__in=[DataEnrichment.STATUS_PENDING, DataEnrichment.STATUS_PARTITION_CREATED]
        )
        ...
        enricher_service = DataEnricherService()
        ...
        for request in pending_requests:
            try:
                ...
                enricher_service.enrich_data(request)
                self.stdout.write(
