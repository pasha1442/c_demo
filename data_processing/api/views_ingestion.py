...
from ..models import DataIngestion, DataIngestionPartition


class DataIngestionPartitionPagination(PageNumberPagination):
    page_size = 10
    page_size_query_param = 'page_size'
    max_page_size = 100


class DataIngestionManager(APIView):
    authentication_classes = [SessionAuthentication, BasicAuthentication]
    permission_classes = [IsAuthenticated]
    pagination_class = DataIngestionPartitionPagination

    def get(self, request):
        """
        Get paginated list of partitioned files for a data ingestion request.
        
        Query Parameters:
        - ingestion_id (required): ID of the data ingestion request
        - status (optional): Filter by partition status
        - page (optional): Page number for pagination
        - page_size (optional): Number of items per page
        """
        try:
            # Get ingestion ID from query params
            ...
            ingestion = get_object_or_404(DataIngestion, id=request_id)
            
            # Set company context
            CompanyUtils.set_company_registry(ingestion.company)

            # Get status filter
            status_filter = request.query_params.get('status', '').lower()

            # Query partitions
            partitions = DataIngestionPartition.objects.filter(
                request=ingestion
            ).order_by('-created_at')
    ...
    def post(self, request):
        ...
        """
        try:
            # Get ingestion ID from request data
            ...
            partitions = DataIngestionPartition.objects.filter(request=ingestion).order_by('-id')
    ...
    def put(self, request):
        """
        Update status of a data ingestion partition.
        ...
        try:
            ...
            # Get partition
            partition = get_object_or_404(DataIngestionPartition, id=partition_id)
