...
from chat.workflow_utils import AgentState, create_agent, create_supervisor_agent, remove_final_answer, router, agent_node
from company.models import Company
from metering.services.openmeter import OpenMeter
from langfuse.decorators import observe
from chat.clients.workflows.icici.tools import search_document_archive, search_catalogue_for_recommendations
...
def create_workflow():
    context = get_context()
    members = ["researcher", "recommender", "FINISH"]


    supervisor_llm_info = get_active_prompt_from_langfuse(context.company.id, "supervisor")
    supervisor_agent = create_supervisor_agent(members, supervisor_llm_info)
    supervisor_node = functools.partial(agent_node, agent=supervisor_agent, name="supervisor", llm_info=supervisor_llm_info)

    researcher_llm_info = get_active_prompt_from_langfuse(context.company.id, "researcher")
    research_agent = create_agent(researcher_llm_info,[search_document_archive])
    research_node = functools.partial(agent_node, agent=research_agent, name="researcher", llm_info=researcher_llm_info)

    recommender_llm_info = get_active_prompt_from_langfuse(context.company.id, "recommender")
    recommendation_agent = create_agent(recommender_llm_info,[search_catalogue_for_recommendations])
    recommendation_node = functools.partial(agent_node, agent=recommendation_agent, name="recommender", llm_info=recommender_llm_info)

    tool_node = ToolNode([search_document_archive,search_catalogue_for_recommendations])
    ...
    workflow.add_conditional_edges(
        "recommender",
        router,
        {"continue": "recommender", "call_tool": "call_tool", "__end__": END},
    )
    workflow.add_conditional_edges(
        "call_tool",
        lambda x: x["sender"],
