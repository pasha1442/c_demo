    ...
    async def generate_summary(self, company, state):
        if state.get('session_id'):
            if state.get('api_route'):
                api_controller = await sync_to_async(ApiController.without_company_objects.get)(api_route=state.get('api_route'), company=company)
            ...
            await summary_generator.generate_summary(company=company, client_identifier=state.get("client_identifier"), mobile=state.get("customer_mobile"), extra_save_data={"session_id":state.get('session_id')})
