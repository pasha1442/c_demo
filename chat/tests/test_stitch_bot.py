...
class TestStitchBot(APIBasedTestCases):

    def setUp(self):
        # This method will run before each test
        self.endpoint = "/api/v1/api-controller/invoke-service/stitch-chat/"
        self.api_url = self.core_server_base_url + self.endpoint
        self.headers = self.get_bot_header(company_name=self.STITCH)
    ...
    def test_auriga_bot(self):
        ...
        if status and status_code == 200:
            print(f"STITCH | STITCH Bot Test | Success {text_response}")
            self.assertEqual(status_code, 200)

        else:
            print(f"STITCH | STITCH Bot Test | Failed | Error : {error_message}")
            self.assertEqual(status_code, 200)
